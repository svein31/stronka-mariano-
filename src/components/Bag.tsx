...
export function Bag() {
 ...
 <motion.div initial={false} animate={{x:open?0:'105%',opacity:open?1:0}} transition={{duration:reducedMotion?0:.28,ease:[.22,1,.36,1]}} ref={surface} className="drawer glass" aria-hidden={!open} data-open={open} role="dialog" aria-modal={open||undefined} aria-labelledby="bag-title" inert={!open} tabIndex={-1} data-lenis-prevent>
 <GlassGlint/><div className="drawer__head"><h2 id="bag-title">Twoje wybory</h2><button onClick={()=>setOpen(false)} aria-label="Zamknij koszyk" className="text-button">Zamknij ×</button></div>
