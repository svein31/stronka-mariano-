...
export function Newsletter() {
 ...
 async function submit(e:FormEvent<HTMLFormElement>) {e.preventDefault();if(busy)return;const data=new FormData(e.currentTarget);setBusy(true);setStatus('');try {await post('/api/newsletter',{email:data.get('email'),consent:data.get('consent')==='on'});setStatus('Zapis zachowany. Wysyłka newslettera i potwierdzeń nie jest jeszcze aktywna.')}catch(e){setStatus((e as Error).message)}finally{setBusy(false)}}
