...
export function ClothStudy({cloth,tint}:{cloth?:Cloth;tint?:string}) {
 ...
 useEffect(()=>{if(!ref.current||!budget.enabled||!('IntersectionObserver' in window))return;const observer=new IntersectionObserver(([entry])=>setNear(entry.isIntersecting),{rootMargin:'240px'});observer.observe(ref.current);return()=>observer.disconnect()},[budget.enabled])
