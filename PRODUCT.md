...
## Order service extension
The measurement guide separates body circumference from garment inseam and transfers tab-local measurements to product guidance. Customer service combines immutable quote versions, explicit decisions, messages, production history and carrier status under the existing expiring order access. Staff roles are enforced by the API. SQLite transactions protect material reservations and daily production capacity. Durable leased jobs process mail, external media, backups and InPost operations; uncertain shipment creation requires reconciliation instead of automatic repeat. Existing payments stay disabled. Serve documented API v1 and preserve existing API aliases.

## Management extension
Add /admin, /track, /personalize/:slug and /newsletter. SQLite becomes the authoritative editable catalog; committed JSON only seeds fresh databases and renders an offline demonstration. Owner controls products, variants, media, measurements, availability and order production events. Passwords are scrypt hashes supplied privately; sessions use HttpOnly cookies, CSRF tokens and server-side expiry. TOTP is mandatory in live mode. Customer tracking uses expiring/revocable bearer tokens and email recovery. Newsletter confirmation/unsubscribe and transactional notifications use a durable encrypted outbox and configured SMTP. No SMTP configuration means queued, not sent. AES-GCM backups use SQLite's backup API, restore verification and private S3 storage; external credentials are operator configuration. Add separate persistent rate limits, trusted-proxy allowlist, redacted audit events, alert hooks and CI gates. All payment endpoints remain disabled.
...
A physical mid-range phone is required to profile real frame time, responsiveness, heat and scrolling. No such test may be claimed from SSR or desktop emulation. Record remaining browser/physical-device checks without claiming measured phone performance. No deployment as part of this change.
